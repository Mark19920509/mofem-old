...
Status Element::defineTypes()
{
    ...
    type_data.num_nodes(TRUSS_NONLINEAR) = 2;
    ...
    type_data.dof_sets(TRUSS_NONLINEAR, 0) = DOF::setCreate(DOF::X, DOF::Y, DOF::Z);
    type_data.dof_sets(TRUSS_NONLINEAR, 1) = DOF::setCreate(DOF::X, DOF::Y, DOF::Z);

    // Number of parameters
    type_data.num_params.resize(TYPE_COUNT);
    type_data.num_params(TRUSS_LINEAR) = 1;     // Area
    type_data.num_params(TRUSS_NONLINEAR) = 1;  // Area
